...
The runnable project lives once at the repo root (the `pulse` package). The build was done in 8 phases; their full snapshots are preserved as git tags (`phase-0` … `phase-7`) and narrated under `phases/` (docs only — no duplicated code). See `docs/Repository-Cleanup-Plan.md` for the restructure rationale.

```
agent/             the pulse package
  config.py          products.yaml + .env loading (pydantic-settings)
  storage.py         SQLite schema + persistence (§5)
  models.py          canonical types — RawReview, Theme, PulseSummary, … (§4.2)
  helpers.py         run_id + ISO-week Window helpers (§7)
  logging_setup.py   structlog with run_id context var
  observability.py   OpenTelemetry spans/metrics
  orchestrator.py    pulse run — chains all stages with resumable checkpoints
  ingestion/         App Store (iTunes RSS) + Play Store scrapers, PII scrub, filters
  clustering/        embeddings + UMAP → HDBSCAN → medoids → KeyBERT
  summarization/     LLM client, prompts, theme/quote/action pipeline
  renderer/          docs_tree.py (batchUpdate tree) + email_html.py
  mcp_client/        MCP sessions + docs_ops / gmail_ops (the only Google surface)
  rest_client/       explicit REST fallback path for Docs/Gmail
services/          the MCP servers, run as subprocesses (own pyproject each)
  docs-mcp/  gmail-mcp/
templates/         email.html, email.txt, doc_section.schema.json
tests/             pytest suite — fixtures/ + mocks/ (mock MCP servers)
docs/              Architecture, Implementation-Plan, runbook, phases/, postmortems/
phases/            docs-only narrative of the 8-phase build journey
products.yaml      per-product config (store ids, distribution list)
pyproject.toml     package "pulse"; entry point `pulse = agent.__main__:app`
.github/workflows/weekly-pulse.yml   weekly cron + manual backfill
```
