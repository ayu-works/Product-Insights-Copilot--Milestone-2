...
body). Phase 5 shifts every index by (doc_end_index - 1) before calling the
API so the new section is appended instead of prepended.
...
Phase 5 (docs.get_document body substring search).
"""
...
def build_doc_requests(
    ...
    for theme in summary.top_themes:
        ...
        # Verbatim quotes — indented italic per architecture §3.1
        ...
        for quote in theme_quotes:
            ...
            _italic(s, e - 1)   # italic all chars except the trailing newline
            _indent(s, e)
    ...
    if summary.what_this_solves:
        ...
        requests.append({
            ...
            # Phase 5 reads _tableContent to fill in cell text after
            # inserting the table structure via the Docs API.
            "_tableContent": {
...
def validate_doc_requests(requests: list[dict[str, Any]]) -> None:
    ...
    Raises ``jsonschema.ValidationError`` if any request is malformed.
    """
    schema = _load_schema()
